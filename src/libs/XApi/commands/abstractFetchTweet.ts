  ...
  implements CacheAble
{
  ...
  // Extract tweet content from the parsed body
  protected extractTweetContent(body: unknown): string | undefined {
    if (!body) return undefined

    try {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const result = this.getResultFromBody(body as any)
      if (E.isLeft(result)) return undefined
      
      const resultValue = result.right
      if (!resultValue) return undefined
      
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const tweetResult = (resultValue as any)?.tweet?.legacy ?? (resultValue as any)?.legacy ?? resultValue
      return tweetResult?.full_text || tweetResult?.text || undefined
    } catch {
      return undefined
    }
  }
  ...
  async resolveResponse(response: Response): Promise<FetchTweetCommandOutput> {
    ...
    try {
      ...
      const tweetResult = this.parseBody(body)
      const tweetContent = this.extractTweetContent(body)
      return {
        $metadata: metadata,
