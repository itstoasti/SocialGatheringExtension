...
const FeatureOptions = ({ featureSettingsRepo }: FeatureOptionsProps) => {
  ...
  return (
    <VStack>
      ...
      />
      <RichFeatureSwitch
        name={i18n('Download tweet text', 'options:features')}
        desc={i18n(
          'Download the tweet text content as a text file alongside media.',
          'options:features'
        )}
        isOn={featureSettings.downloadTweetText}
        handleClick={toggler.tweetText}
        testId="tweetText-feature-switch"
      />
    </VStack>
  )
}
