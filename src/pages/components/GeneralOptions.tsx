...
import { Button, Flex, HStack, Input, Select, VStack, Divider, Heading, Box } from '@chakra-ui/react'
import {
  DndContext,
  KeyboardSensor,
  PointerSensor,
...
const GeneralOptions = (props: GeneralOptionsProps) => {
  ...
  return (
    <Box width="100%" maxW="none">
      <VStack spacing={6} align="stretch" width="100%" maxW="none">
        <form
        onSubmit={e => {
          e.preventDefault()
          formHandler.submit()
      ...
      <VStack spacing={6} align="stretch">
        <Heading size="md">{i18n('Features', 'options:sideMenu')}</Heading>
        <FeatureOptions featureSettingsRepo={props.featureSettingsRepo} />

        <Divider />

        <Heading size="md">{i18n('Integrations', 'options:sideMenu')}</Heading>
        <IntegrationOptions
          downloadSettingsRepo={props.downloadSettingsRepo}
          warningSettingsRepo={props.warningSettingsRepo}
        />
      </VStack>
      </VStack>
    </Box>
