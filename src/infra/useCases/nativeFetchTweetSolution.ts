  ...
  implements FetchTweetSolutionWithTransactionId<StatisticIdentity>
{
  ...
  public _lastFetchResult?: { tweetContent?: string }
  ...
  execCommand(CommandConstructor: FetchCommandConstructor) {
    return async (config: {
      ...
      // Store the tweet content for later use
      if (value?.tweetContent) {
        this._lastFetchResult = { tweetContent: value.tweetContent }
      }
